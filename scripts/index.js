    ...
    .then((ogg) => {
      ogg.data.forEach((songs) => {
        ...
        <div class="d-flex">
                  ...
                  <div>
                   <a class="text-decoratione-none " href="../album.html?albumId=${songs.album.id}" ><h6 class="d-inline-block text-white mb-1">${songs.album.title}</h6></a>
                   <a class="text-decoratione-none link-underline link-underline-opacity-0 "href="../artist.html?artistId=${songs.artist.id}"><p style="color: #6c757d" class="mb-0">${songs.artist.name}</p></a>
                  </div>
                </div>
        `;
        ...
        cardSectionListenNow.innerHTML = `
              <div class="card border-0" style="background-color: #212121">
                  ...
                  <div class="card-body px-0">
                   <a class="text-decoratione-none link-underline link-underline-opacity-0" href="../album.html?albumId=${songs.album.id}" > <p class="card-text custom-grey">${songs.album.title}</p></a>
                  </div>
                </div>
              `;
